...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    if args.resolution in [1, 2, 4, 8]:
        ...
        # [修复 1: 为该分支补充 HWK 的计算]
        HWK = None
        if hasattr(cam_info, 'K') and cam_info.K is not None:
            K = cam_info.K.copy()
            K[:2] = K[:2] / scale
    ...
    # [修复 2: 如果数据集没有自带 K 矩阵，强行通过相机 FOV 计算，彻底免疫报错]
    if HWK is None:
        width, height = resolution[0], resolution[1]
        fx = fov2focal(cam_info.FovX, width)
        fy = fov2focal(cam_info.FovY, height)
        K = np.array([[fx, 0, width / 2.0], [0, fy, height / 2.0], [0, 0, 1]], dtype=np.float32)
        HWK = (height, width, K)

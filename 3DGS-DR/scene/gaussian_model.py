...
class GaussianModel:
    ...
    def __init__(self, sh_degree = -1):
        ...
        self.env_map = None
        self.env_map_trans = None # [新增] 透射贴图
    ...
    # [新增] 获取透射贴图
    @property
    def get_envmap_trans(self): 
        return self.env_map_trans
    ...
    def create_from_pcd(self, pcd, spatial_lr_scale: float, cubemap_resol = 128):
        ...
        # [新增] 初始化透射贴图
        env_map_trans = CubemapEncoder(output_dim=3, resolution=cubemap_resol)
        self.env_map_trans = env_map_trans.cuda()
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            # [新增] 把透射贴图加入优化器
            {'params': self.env_map_trans.parameters(), 'lr': training_args.envmap_cubemap_lr, "name": "env_trans"}
        ]
    ...
    def save_ply(self, path):
        ...
        if self.env_map is not None:
            ...
            # [新增] 保存透射贴图
            save_trans_path = path.replace('.ply', '.map_trans')
            torch.save(self.env_map_trans.state_dict(), save_trans_path)
    ...
    def load_ply(self, path):
        ...
        # [新增] 加载透射贴图
        map_trans_path = path.replace('.ply', '.map_trans')
        if os.path.exists(map_trans_path):
            self.env_map_trans = CubemapEncoder(output_dim=3, resolution=128).cuda()
            self.env_map_trans.load_state_dict(torch.load(map_trans_path))
    ...
    def _prune_optimizer(self, mask):
        ...
        for group in self.optimizer.param_groups:
            ...
            # [修改] 把 env_trans 也跳过，因为它是全局贴图，不随高斯点云分裂而增加
            if group["name"] in ["mlp", "env", "env_trans"]: continue
            stored_state = self.optimizer.state.get(group['params'][0], None)
